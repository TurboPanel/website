...
- **`editOnGithub`** on docs pages and the MDX `<File>` chip both use **`DOCS_GITHUB`** in `src/lib/docs-github.ts` (`turbopanel/turbopanel-website` on branch **`trunk`**); paths are `docs/…` (no monorepo prefix).
