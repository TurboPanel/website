...
On every open the self-hosted store checks the sidecar `schema-version` marker against `DUCKDB_SCHEMA_MARKER_VERSION` (**6**). A missing, corrupt, or non-6 marker discards `metrics.duckdb`, `parquet/`, `tmp/`, and `schema-version` before creating the current layout. There is no in-place migration and no supported operator path for older DuckDB files.
...
| schema marker (sidecar) | Written after a successful open of schema **6** (`DUCKDB_SCHEMA_MARKER_VERSION`) |
