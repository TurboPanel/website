...
| Expectation | Detail |
| ----------- | ------ |
| Runtime | Docker container `turbopanelch` (official `clickhouse/clickhouse-server` image) on the `turbopanel` network |
...
| Data path | Named Docker volume `turbopanelch` (`/var/lib/clickhouse` in-container) |
