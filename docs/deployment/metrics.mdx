...
| Expectation | Detail |
| ----------- | ------ |
| Runtime | Docker container `turbopanela` (official `clickhouse/clickhouse-server` image) on the `turbopanel` network |
...
| Data path | Named Docker volume `turbopanela` (`/var/lib/clickhouse` in-container) |
