...
Production issues typically fall into TLS/Caddy misconfiguration, instance socket permissions, daemon WebSocket connectivity, Postgres socket access, or Ansible playbook failures. The self-hosted recovery address is **HTTPS on port 8443** (Platform CA). A Let's Encrypt or uploaded hostname is also served on **:443**. Let's Encrypt's HTTP-01 check arrives on **:80** and is answered on **:8880**. On co-located dev hosts, Caddy also serves a plaintext mirror at **`http://<host>:8880`** when **`TURBOPANEL_DEV_HTTP_CONTROL_PLANE=1`**. A managed install binds that same port for the solver, and every other path there redirects to `:8443`.

| Port | When |
| ---- | ---- |
| **8443** | Platform CA recovery address, always |
| **8880** | HTTP-01 solver on a managed host. Plaintext mirror on co-located dev only |
| **443** | Each Let's Encrypt or uploaded hostname |
| **80** | Public edge. Forwards the challenge path to `:8880` |

## Port conflicts (8443 / 8880 / 80 / 443)

**Problem:** Caddy cannot bind `:8443` or `:8880`. A Let's Encrypt or uploaded hostname also needs `:443` free on control-plane Caddy. Port **80** belongs to whoever owns the public edge (hosting Caddy, or an external forward to `:8880`).
...
**Problem:** Browser shows errors at `https://<host>:8443`, at `https://<hostname>/` for a Let's Encrypt or uploaded name, or (co-located dev) `http://<host>:8880`.
...
1. Verify instance URL: `TURBOPANEL_INSTANCE_URL=https://<host>:8443`. A Let's Encrypt or uploaded hostname is also `https://<hostname>/`. On co-located dev only, `http://<host>:8880` when `TURBOPANEL_DEV_HTTP_CONTROL_PLANE=1` (no CA required)
...
| Daemon WS path (HTTPS) | `wss://<host>:8443/ws/daemon/v1`. A Let's Encrypt or uploaded hostname is also `wss://<hostname>/ws/daemon/v1` |
