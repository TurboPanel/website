...
5. Firewall: allow **8443** by default from remote daemons to instance (**443** in `lets_encrypt`; **8880** is dev-only and not for remote production nodes). A host that shipped with `ufw` active blocks 8443 by default while `curl https://127.0.0.1:8443/api/health` still answers locally — `ufw status` will show only `22/tcp`; `ufw allow 8443/tcp` opens it. The installer does not do this yet (managed firewall: first 0.1.x push).
