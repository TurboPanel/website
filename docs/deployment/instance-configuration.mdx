...
| Variable | Default | Behaviour |
| -------- | ------- | --------- |
| `TURBOPANEL_PUBLIC_URLS` | derived | Flat projection of the `platform-ca` hostnames. Seeds those SANs and the install-command host. **Admin → Access → Hostnames** is the editor; this variable stays in sync so older readers keep working. Unset, the instance derives an origin from the host's interfaces. |
...
| `TURBOPANEL_TLS_PUBLIC` | unset | `1` / `true` when a leaf is publicly trusted (any Let's Encrypt hostname, or an uploaded public cert): the daemon CA route `GET /api/daemon/v1/instance/ca` then 404s and install commands omit `--insecure-tls`. `:8443` still presents the Platform CA. |
...
| `TURBOPANEL_TLS_CERTS_DIR` | `<state>/tls/certs` (compiled) / `<checkout>/certs` (source) | Where the Platform CA leaf (`platform-ca.*`, with `self-signed.*` linked to it) and uploaded pairs are written. |
| `CADDY_PORT` | `8443` | The Platform CA recovery listener. Always bound. A Let's Encrypt or uploaded hostname is an extra site on port 443. |
...
| `CADDY_TLS_CERT`, `CADDY_TLS_KEY` | `<certs dir>/self-signed.crt`, `.key` | Leaf paths the **development** Caddyfile reads. A managed Caddyfile bakes `platform-ca.*` and per-hostname files at render time. |
| `TURBOPANEL_TRUSTED_PROXY_CIDRS` | `127.0.0.0/8,::1/128` | Peers whose `CF-Connecting-IP` / `X-Forwarded-For` the instance believes. The value **replaces** the loopback default. **Admin → Access → Trusted proxies** shows the effective list and does not edit it. |
| `TURBOPANEL_INSTANCE_ACME__CONTACT_EMAIL` | unset | Contact email for this control plane's Let's Encrypt account. |
| `TURBOPANEL_INSTANCE_ACME__TOS_ACCEPTED` | `false` | Terms acceptance for that account. |
| `TURBOPANEL_INSTANCE_ACME__DIRECTORY_URL` | Let's Encrypt production directory | ACME directory URL. |
| `TURBOPANEL_INSTANCE_ACME__USE_STAGING` | `false` | `true` selects the Let's Encrypt staging directory. |
...
Every `SYSTEM_EMAIL` setting has an environment form `TURBOPANEL_SYSTEM_EMAIL__<KEY>` that wins over the value stored in the admin settings: `PROVIDER` (`smtp` default, `mailgun`, or `mailpit` for development), `FROM` (`noreply@turbopanel.local`), `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `MAILGUN_API_KEY`, `MAILGUN_DOMAIN`, `MAILGUN_REGION`, `RATE_LIMIT_PER_MINUTE`, `RATE_LIMIT_BURST`, `QUEUE_PREFETCH`. Secrets stored in the database are `tpsecret` envelopes sealed with the root keyring. The email consumer runs inside the instance process, reads the same variables and reaches whatever host `SMTP_HOST` names.
...
Read only when the developer surface is on (`TURBOPANEL_DEV_SURFACE=1`, written solely by the managed `turbopanel-instance` unit for co-located Deno source-mode development; `TURBOPANEL_UI_MODE` only selects Expo vs. static serving), never on a managed install: `TURBOPANEL_DEV_USER`, `TURBOPANEL_DEV_HOST_AUTH`, `TURBOPANEL_DEV_HTTP_CONTROL_PLANE`, `TURBOPANEL_TRUNK_BRANCH`, `TURBOPANEL_DAEMON_REPO` / `TURBOPANEL_UI_REPO` (optional checkout overrides; defaults derive from `TURBOPANEL_DEV_ROOT`), `TURBOPANEL_UI_MODE=dev`, `TURBOPANEL_DRIZZLE_STUDIO_HOST` / `_PORT`, `TURBOPANEL_NODE`, `TURBOPANEL_DENO`, `TURBOPANEL_RUNTIMES_DIR`, `TURBOPANEL_USER`, `CADDY_HTTP_PORT`. See [Local development](/docs/getting-started/development).
