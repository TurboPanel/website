...
| `TURBOPANEL_UPDATE_CHANNEL` | The channel this instance resolves daemon, control-plane, and UI updates on (`trunk` default; `canary`, `rc`, `release`) |
| `TURBOPANEL_UPGRADE_STEP_RETENTION_DAYS` | How long a successful upgrade step is kept (default 14). Failed steps stay 90 days; runs stay 365 days, newest 50 kept. See [Instance configuration](/docs/deployment/instance-configuration) |
...
**Admin → Updates** is the managed platform upgrade. On self-hosted it runs this host's daemon, then the control plane, then the other servers, and the fleet waits until the first two are on target. On TurboPanel High Availability the control plane line is **Managed by TurboPanel** and only daemons roll out. Detail: [Upgrade and rollback](/docs/deployment/upgrade).
