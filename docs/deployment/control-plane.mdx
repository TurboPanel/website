...
| Unit | User | Role |
| ---- | ---- | ---- |
| `turbopaneld.service` | `turbopanel` (9999) | Ansible orchestration (daemon from source checkout in dev) |
| `turbopanel-instance.service` | `instance` (9998) | Deno API on Unix socket |
| `turbopanel-caddy.service` | `instance` | TLS + reverse proxy `:8443`; dev-only plaintext mirror `:8880` when enabled |
