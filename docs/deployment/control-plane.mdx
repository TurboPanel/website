...
The Caddy site config is rendered by the daemon's `instance-launch` role into `/etc/turbopanel/caddy/Caddyfile` on every converge — one template whose branches are the three modes. It is never hand-edited; re-run the installer or converge to change it.

| Mode | Rendered site block | Listen | Leaf material | `GET /api/daemon/v1/instance/ca` | `--insecure-tls` |
| --- | --- | --- | --- | --- | --- |
| `self_signed` (default) | port + platform-CA leaf, `auto_https off` | `:8443` | `certs/self-signed.*` | 200 | yes |
| `upload` | port + operator pair, `auto_https off` | `:8443` (or `caddy_port`) | operator pair copied to `certs/` | 200 unless `turbopanel_tls_public` | follows `turbopanel_tls_public` |
| `lets_encrypt` | public hostname, automatic HTTPS | `:443` (+ `:80` challenge) | Caddy ACME storage | 404 | no |
