...
## Compose deploys (project + environment layers)

Docker Compose projects do **not** ship a single pre-merged compose file to the daemon as the deploy source of truth. The control plane stores a **project** base compose and an optional **environment** overlay, then prepare/deploy writes each as its own file in an ordered multi-file chain:

`docker compose -f <project> -f <environment> -f <platform> …`

Order is **project → environment → (optional future repository layer) → platform last**. Platform is always daemon/instance-authored (labels, secrets, storage inject) and appears **at most once**, last. This matches the multi-file model native to `docker compose`, so precedence follows the `-f` order rather than a silent full rewrite into one file.

Overrides and nullification between layers follow the [Compose Spec merge rules](https://docs.docker.com/reference/compose-file/merge/): mapping keys merge recursively (a later layer wins per key), most sequences — `ports`, `volumes`, `env_file`, and similar lists — **append** with attribute-specific key-based de-duplication rather than replacing the earlier layer wholesale, and `command`, `entrypoint`, and a service's `healthcheck.test` always **fully replace** rather than append. Authors who need to remove a value entirely, or force full replacement instead of the default append/merge behavior for a sequence, can use the reserved **`!reset`** (delete the key) and **`!override`** (force full replacement) YAML tags on the environment overlay.

In the organization console, **Merged compose** is a client-side simulation of the fully merged effective document (readability aid). **Prepared compose** and the live `environment.deploy` path use the same ordered `composeFiles[]` `-f` chain; a deprecated merged `composeYaml` field remains only as a display/legacy fallback. See also [API architecture — multi-file compose layers](/docs/architecture/api#multi-file-compose-layers--f-chain).
