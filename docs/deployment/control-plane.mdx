...
- WebSocket hub for agent daemons (`/ws/daemon/v1`)
...
<Callout type="info" title="Current production path">
  There is no public one-line install script for self-hosted control planes yet. Use **`develop.sh`**
  on a Debian/Ubuntu VM with sibling `instance/` and `daemon/` checkouts (clone via
  [turbopanel-dev](https://github.com/turbopanel/turbopanel-dev) `pull.sh` or manual layout).
</Callout>

On Debian/Ubuntu with `instance/` and `daemon/` checkouts:
...
This enables co-located dev-instance mode and lets the daemon run `instance-dev-install` via Ansible. Services:
...
Configure Hyperdrive, secrets (`TURBOPANEL_SECRET` / `TURBOPANEL_SECRETS`), and bindings in `wrangler.jsonc` before deploy.

<Callout type="warn" title="Edge coordination not complete">
  **Durable Objects** for cloud-hosted daemon coordination are **not implemented yet**. Deployed
  Workers instances and remote daemons use the same HTTPS/WSS paths as self-hosted (`/api/daemon/v1/*`,
  `/ws/daemon/v1`) until edge-specific coordination ships.
</Callout>

### Contributor local dev (Tilt)
...
Defaults to Workers (`wrangler dev`) behind Caddy — see [Local development](/docs/getting-started/development).
...
| `TURBOPANEL_PG_*` | Postgres socket connection (self-hosted) |
...
Local Tilt copies these from `dev/.env` via `scripts/sync-env.sh`. Managed hosts inject vars via Ansible `instance-launch`.
...
| `/api/admin/v1/*` | Reserved admin surface |
