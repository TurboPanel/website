...
The Caddy site config is rendered by the daemon's `instance-launch` role into `/etc/turbopanel/caddy/Caddyfile` on every converge. It is never hand-edited. `turbopanel_tls_mode` is a display-only derivation of the hostname list (an uploaded name, otherwise a Let's Encrypt name, otherwise the Platform CA). An empty hostname list is still synthesized from the older single-name aliases so an existing converge keeps working.

| Listener | Always | What it serves |
| --- | --- | --- |
| **:8443** | yes | Platform CA leaf. Recovery address for the wizard, browsers, and daemons. Covers every `platform-ca` hostname as a SAN, and stays up when none are configured. |
| **:8880** | yes | HTTP-01 solver for control-plane Let's Encrypt. Any other path redirects to `:8443`. |
| **:443** | for each `uploaded` or `lets-encrypt` hostname | That name's own site. Let's Encrypt uses Caddy automatic HTTPS. An uploaded pair is `tls` with that certificate's files. |
| **:80** | whoever owns the public edge | Let's Encrypt dials this port. It is forwarded to the solver on `:8880`. See [Hostnames and TLS](/docs/deployment/hostnames-and-tls). |

`TURBOPANEL_TLS_PUBLIC=1` (set when any hostname is Let's Encrypt, or when `turbopanel_tls_public` is true) makes the Deno CA route 404 so daemons use the system trust store, and install commands omit `--insecure-tls` even on a non-443 port. The Platform CA is still minted, and `:8443` still presents it.

Co-located development loads a different Caddyfile. There `:8880` is a plaintext mirror of `:8443`, gated by `TURBOPANEL_DEV_HTTP_CONTROL_PLANE=1`. That file has no per-hostname sites, so a development host refuses a Let's Encrypt hostname.
...
<Steps>
  ...
  <Step>
    Complete the install wizard at `https://<host>:8443` (host PAM + superadmin). That address stays bound. The certificate is signed by the Platform CA at `/var/lib/turbopanel/tls/ca.crt`. A hostname you later publish as Let's Encrypt or an uploaded pair is also served on `https://<hostname>/` once that certificate is in place.
  </Step>
  <Step>
...
| `CADDY_PORT` | Platform CA recovery listener (default **8443**). A Let's Encrypt or uploaded hostname is an additional site on **443**. |
| `TURBOPANEL_TLS_PUBLIC` | When `1` / `true`, Deno `GET /api/daemon/v1/instance/ca` 404s and install commands omit `--insecure-tls`. Set when any hostname is Let's Encrypt. |
| `CADDY_HTTP_PORT` | Solver / redirect listener on a managed host (default **8880**). On co-located dev this same port is the plaintext mirror. |
| `TURBOPANEL_DEV_HTTP_CONTROL_PLANE` | Must be `"1"` before the instance accepts the dev plaintext mirror on `CADDY_HTTP_PORT`; injected automatically on co-located dev hosts only |
| `TURBOPANEL_INSTANCE_ACME__CONTACT_EMAIL`, `…__TOS_ACCEPTED`, `…__DIRECTORY_URL`, `…__USE_STAGING` | Control-plane Let's Encrypt settings. Env wins over **Admin → Access → Certificates**. Independent of every organization's Let's Encrypt opt-in. |
| `TURBOPANEL_TRUSTED_PROXY_CIDRS` | Peer addresses whose forwarding headers the instance believes. **Replaces** the loopback default (`127.0.0.0/8`, `::1/128`). Include loopback when Caddy is still on this host. |
...
**HTTPS entrypoint (self-hosted):** **`https://<host>:8443`** is always the Platform CA recovery address. A Let's Encrypt or uploaded hostname is also served at **`https://<hostname>/`** on `:443`.

<Callout type="warn" title="Dev-only plaintext HTTP mirror">
  Co-located development exposes a plaintext mirror at **`http://<host>:8880`**
  (`CADDY_HTTP_PORT`, default **8880**). The instance accepts it only when
  **`TURBOPANEL_DEV_HTTP_CONTROL_PLANE=1`** (set automatically by Ansible when
  ...
  the flag is off. A managed install binds the same port for a different job:
  the Let's Encrypt solver, with every other path redirecting to `:8443`. It is
  not a second copy of the panel. Both **`TURBOPANEL_INSTANCE_RUNTIME=deno`**
  and **`=workers`** use the same development Caddyfile, so the dev mirror
  covers every route on `:8443` for either runtime.
</Callout>
