...
both follow a **channel** — `release` (the latest promoted release) or `rc` (the current
release candidate) — and resolve that channel's `manifest.json` straight from GitHub. There
is no self-update inside the control plane: upgrading it means running the installer again.
