...
<Callout type="info" title="First upgrade onto this release">
  ...
  **only the daemon**, pinned to one verified manifest. On a host that already runs
  ...
  Replace `<tag>` with the daemon release you are upgrading to, as listed on the daemon's
  [GitHub Releases](https://github.com/TurboPanel/turbopaneld/releases) (for example
...
Pin a remote node's daemon. On a node enrolled with a self-hosted control plane, also pass `TURBOPANEL_HOST` with that control plane's URL, as in [Refresh a stuck daemon](/docs/deployment/daemon-update); without it the node is enrolled with the hosted control plane:
...
Pins accept release manifests only. Each package's pin must be a `manifest.json` or `manifest-<version>.json` under its own repository's GitHub Releases (`https://github.com/TurboPanel/<repo>/releases/download/<tag>/…` or `…/releases/latest/download/…`); the daemon also accepts its CDN channel drop (`https://dl.trbp.nl/channels/<channel>/manifest.json`). The installer and the root helper refuse anything else, including a URL with `..`, a percent-encoded character, a port or a query string. A `TURBOPANEL_*_MANIFEST_URL` pin in `daemon.env` that does not match is ignored, as if unset.
