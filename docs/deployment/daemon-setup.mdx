...
### Refresh an existing node (`update.sh`)

If the server is already installed, run **`update.sh` from the checkout** on the
server (not a CDN URL — unlike `run.sh`). It reads the license from state and
the channel from `.env`:

```bash
sudo sh /opt/turbopanel/platform/daemon/scripts/update.sh
```

Switch channel (written back to `.env` on success):
