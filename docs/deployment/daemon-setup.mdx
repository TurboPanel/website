...
If the server is already installed, use **`update.sh`** instead of hunting for the
license. It reads credentials from the daemon state directory and the update
channel from `.env`:
