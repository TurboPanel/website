...
- Connect to the instance control plane as an agent node
...
<Callout type="info" title="Control plane host">
  On the machine that runs the instance, use `develop.sh` or Ansible co-located install — not the
  remote node installer.
</Callout>
...
The installer bootstraps uv/Python/Ansible and runs `agent-install.yml`:
