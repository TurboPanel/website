...
| ------- | ---------------------------- | -------------------- |
| Instance URL | Hosted Workers URL (manifest default) | `https://<host>:8443` always. A Let's Encrypt or uploaded hostname is also `https://<hostname>/` on `:443`. |
...
`:8443` is always the Platform CA recovery address. Each published hostname uses the Platform CA, an uploaded pair, or Let's Encrypt. Let's Encrypt answers HTTP-01 on port 80 by forwarding the challenge to `:8880`. Full listeners: [Control plane](/docs/deployment/control-plane#control-plane-listeners) and [Hostnames and TLS](/docs/deployment/hostnames-and-tls).
...
Self-hosted examples below use `:8443`. A Let's Encrypt or uploaded hostname is also reachable on `:443`. Public TLS omits `--insecure-tls`.
