...
`lets_encrypt` needs `:80`/`:443` free on the control-plane host. Full table: [Control plane](/docs/deployment/control-plane#control-plane-tls-modes).
...
| `TURBOPANEL_UPDATE_CHANNEL` / `--channel <name>` | Release channel. Default: `trunk`. Channel names (`trunk`, `canary`, `rc`, `release`, …) are technical identifiers — not product branding. `release` and `rc` resolve from the daemon's [GitHub Releases](https://github.com/TurboPanel/turbopaneld/releases) (latest non-pre-release, and the rolling `rc` pre-release); `trunk` from the per-merge build drop. Written to `.env` as `TURBOPANEL_UPDATE_CHANNEL`. |
