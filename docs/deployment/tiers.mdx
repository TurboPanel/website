---
title: License tiers
description: The S1–S7/SX ladder on TurboPanel High Availability — how a server is placed, what a tier watches, what happens when a machine outgrows its license, and why self-hosted has none of this
---

# License tiers

On **TurboPanel High Availability**, every connected server sits on one tier, **S1** to **S7**, with **SX** above the ladder for machines that do not fit. The tier is set by the **machine**, not by a feature list: cores and RAM decide where a server must sit, and the tier in turn decides how many of that server's devices the metrics pipeline watches. Planned prices are on [turbopanel.io/pricing](https://turbopanel.io/pricing).

<Callout type="info" title="Self-hosted has no tiers">
  A self-hosted control plane is free and uncapped. It never truncates a sample, never places a
  server on a tier, and never sends a tier notice. Everything on this page is about TurboPanel High
  Availability. See [Self-hosted](/docs/deployment/self-hosted).
</Callout>
...
The first two columns are the **hard floor**. The rest are **watched-device counts** — the slot entitlements the tier row carries, mapped onto the server's metrics capability plan. Tiers are rows in the platform catalogue, not an enum: a re-price is a new generation of rows, and an existing license keeps pointing at the row it was sold on.
...
**Below the required tier — access is refused.** When a server enrolls, the control plane evaluates the hard floor against the hardware the daemon reports. A license whose tier ranks below the required tier is refused with `License tier below required` (a license with no tier at all gets `License tier not assigned`); the daemon treats both as permanent and parks. The same check runs at **session issuance**, so a server whose cores or RAM grow past its tier after a resize loses access on its next session rather than mid-stream — the metrics ingest path and the WebSocket hello never apply the floor, because refusing samples would hide an incident rather than bill for it. Assigning a higher tier to the license restores access on the next session.
