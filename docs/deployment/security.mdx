...
`:8443` always presents the Platform CA. A publicly trusted hostname (Let's Encrypt, or `TURBOPANEL_TLS_PUBLIC`) makes `GET /api/daemon/v1/instance/ca` answer 404 so daemons use the system trust store, and install commands omit `--insecure-tls`. See [Control plane listeners](/docs/deployment/control-plane#control-plane-listeners).

Control-plane Caddy sends a baseline response-header set on every response: `Strict-Transport-Security`
...
- Implement firewall rules to restrict access to the HTTPS entrypoint (**8443** always; **443** for each Let's Encrypt or uploaded hostname)
...
- Allow inbound to the HTTPS entrypoint (**8443** always; **443** for each Let's Encrypt or uploaded hostname; **80** forwarded to the solver on **8880** when a hostname uses Let's Encrypt) only from trusted networks
...
- Use TLS with the Platform CA (or a public certificate) for all daemon ↔ instance traffic. Public TLS (Let's Encrypt / `TURBOPANEL_TLS_PUBLIC`) uses the system trust store and omits `--insecure-tls`.
...
# :8443 always presents the Platform CA.
# A Let's Encrypt or uploaded hostname also binds :443 and omits --instance-ca / --insecure-tls.
