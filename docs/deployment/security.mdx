...
- Rotate `TURBOPANEL_SECRET` / `TURBOPANEL_SECRETS` on the instance per your key-rotation policy
...
- **Firewall Rules:** Daemons are outbound clients — block unnecessary inbound to agent hosts; allow outbound **8443** (or your instance URL) from agents to the control plane
...
# Remote agent node — official installer
...
```bash
# Remote agent node connecting to Workers instance
curl -fsSL https://raw.githubusercontent.com/turbopanel/turbopanel-cdn/trunk/install.sh \
  | sudo bash -s -- \
...
<Callout type="info" title="Daemon hardening">
  Daemons dial outbound to the instance — no inbound listener is required on agent nodes for
  control-plane connectivity. Mount the Docker socket read-only (`:ro`) only when the daemon truly
