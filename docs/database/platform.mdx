...
description: "Data dictionary — access, audit and configuration tables: audit, certificate, grant, lease, origin, setting"
---
...
Tables on this page: [`audit`](#audit) · [`certificate`](#certificate) · [`grant`](#grant) · [`lease`](#lease) · [`origin`](#origin) · [`setting`](#setting). Generated from migration `0007_add_upgrade_tables`; see [How to read this](/docs/database#how-to-read-this-dictionary).
...
| `id` (PK) | `uuid` | no | `uuidv7()` | Primary key (`uuidv7()`, time-ordered). |
| `created_at` | `timestamp(3) with time zone` | no | `now()` | Row creation time. |
| `label` | `text` | no |  | Operator-chosen name for the uploaded pair, shown in the admin certificate list. |
| `cert_pem` | `text` | no |  | Public certificate PEM, chain allowed; the leaf is parsed for names and expiry when the pair is stored. |
| `key_pem` | `text` | no |  | Private key PEM sealed as a `tpsecret` envelope; the admin API never returns it. |
| `dns_names` | `jsonb` | no |  | JSON array of DNS names and IP addresses parsed from the leaf, used to test which hostnames the pair covers. |
| `not_after` | `timestamp(3) with time zone` | no |  | Leaf expiry copied from the parsed certificate when the pair is stored. |
...
| `created_at` | `timestamp(3) with time zone` | no | `now()` | Row creation time. |
| `updated_at` | `timestamp(3) with time zone` | no | `now()` | Last write time; the ORM sets it to `now()` on every update. |
| `host` | `text` | no |  | Normalized public URL entry (origin, host, or host:port) in the form public URL parsing already stores. |
| `source` | `text` | no |  | Certificate source for this name: `platform-ca`, `uploaded`, or `lets-encrypt`. |
| `uploaded_cert_id` | `uuid` | yes |  | FK → [`certificate`](/docs/database/platform#certificate).`id` (on delete set null). Uploaded pair (`certificate.id`) this name serves; required when `source` is `uploaded`, otherwise NULL. |
| `acme_last_attempt_at` | `timestamp(3) with time zone` | yes |  | When this instance last tried Let's Encrypt for this name; NULL until an attempt runs. |
| `acme_last_error` | `text` | yes |  | Last Let's Encrypt error for this name; NULL when the last attempt succeeded or none has run. |
| `not_after` | `timestamp(3) with time zone` | yes |  | Leaf expiry for this name when known; NULL for `platform-ca` and for Let's Encrypt before a leaf exists. |

**Constraints and indexes**

- Unique `origin_host_unique`: (`host`)
- Index `idx_origin_uploaded_cert_id`: (`uploaded_cert_id`)
- Check `origin_source_check`: `source IN ('platform-ca', 'uploaded', 'lets-encrypt')`
- Check `origin_uploaded_cert_check`: `(source <> 'uploaded') OR (uploaded_cert_id IS NOT NULL)`
