...
Tables on this page: [`2fa`](#2fa) · [`account`](#account) · [`passkey`](#passkey) · [`session`](#session) · [`user`](#user) · [`verification`](#verification). Generated from migration `0004_schema_comments`; see [How to read this](/docs/database#how-to-read-this-dictionary).
