...
The dataset name is **`turbopanel_server_metrics_v4`**. The Wrangler binding is **`SERVER_METRICS_V4`**.
...
AE doubles have no null. Missing metrics store **`AE_V4_MISSING_METRIC_SENTINEL = -1e308`**. All host metrics are ≥ 0, so the sentinel cannot collide. On the AE SQL read path, compare against **`-pow(10, 308)`**.
