...
Scheduling: **60 s** interval, ≤**5 s** deterministic per-`serverId` phase jitter, monotonic process-local `sequence` (resets on daemon restart). Independent of cell ping/`heartbeat` liveness — see daemon `AGENTS.md`. The daemon WS `{ type: "metrics" }` frame path is a deprecated transition fallback only.
