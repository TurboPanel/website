...
TurboPanel collects **host metrics** from connected daemons over `/ws/daemon/v1` — approximately one sample per minute per server. Metrics is **disposable, statistical, and may be sampled**; it is not a billing ledger or audit trail.
...
```mermaid
graph LR
    Daemon["turbopaneld"] -->|"{ type: metrics } ~1/min"| WS["/ws/daemon/v1"]
    WS --> Workers["Workers DO validate + write"]
    WS --> Deno["Deno WS validate + write"]
    Workers --> AE["Analytics Engine"]
    Deno --> CH["ClickHouse"]
...
```json
{
  ...
  "dimensions": { "daemonVersion": "…", "operatingSystem": "…", "architecture": "…", "kernelRelease": "…" }
}
```

Scheduling: **60 s** interval, ≤**5 s** deterministic per-`serverId` phase jitter, monotonic process-local `sequence` (resets on daemon restart). Independent of cell ping/`heartbeat` liveness — see daemon `AGENTS.md`.
...
AE doubles have no null. Missing metrics store **`AE_MISSING_METRIC_SENTINEL = -1e308`** — never `0`. All host metrics are ≥ 0, so the sentinel cannot collide.
...
```sql
SUM(if(doubleN = -1e308, 0, _sample_interval * doubleN))
/ NULLIF(SUM(if(doubleN = -1e308, 0, _sample_interval)), 0)
```
...
Database `turbopanel_metrics`; DDL is idempotent (`ensureSchema` once per process).
...
**Late arrivals / duplicates:** all inserts accepted (no `ReplacingMergeTree` / `FINAL`). Legacy `host_metrics_raw` / rollup objects are removed during admin-owned converge migrations.
