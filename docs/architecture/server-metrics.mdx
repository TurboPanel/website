...
| `double1` … `double20` | Host rows (`blob1 = "host"`): metrics 1–20 in table order above |
| `blob1` | Event type discriminator — `"host"` (metrics sample) or `"status"` (connection-status transition; see below) |
...
| `blob7` … `blob20` | Reserved empty strings on host rows; `blob7` carries the transition reason on status rows |
...
| `blob7` | Transition reason — `"connect"` \| `"disconnect"` \| `"sweep_stale"` \| `"self_heal"` (closed set) |
| `double1` | `1` (connected) or `0` (disconnected) |
| all other doubles/blobs | Missing-metric sentinel / empty string — a status row carries no host metrics |
...
| `GET /api/client/v1/servers/:id/metrics/connection` | Uptime/downtime history from the `blob1 = "status"` event stream (history-only — see above) |
