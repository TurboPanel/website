...
- **Every telemetry store has a disabled fallback.** `resolveExecutionLogStore` and `resolveServerMetricsStore` return a safe no-op store rather than throwing when their backend is unconfigured, so a half-converged deployment still serves commands. **Callers never branch on availability.**
