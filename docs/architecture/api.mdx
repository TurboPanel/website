...
### Multi-file compose layers (`-f` chain)

TurboPanel does not merge the project and environment compose documents into a single file before deploying. It writes each as its own file — project layer, environment overlay layer, and a platform-authored layer appended last — and runs them together as an ordered `docker compose -f project.yml -f environment.yml -f platform.yml …` chain (project → environment → platform, platform always last and at most one). This is the same multi-file model `docker compose` supports natively, so precedence is exactly what the `-f` order implies — there is no separate merge step that silently changes behavior between what is authored and what is deployed.
...
The organization console's **Merged compose** preview renders a client-side simulation of the fully merged result — useful for reviewing the effective configuration at a glance — while the **Prepared compose** view and the daemon's actual deploy run the real ordered multi-file `-f` chain described above. The two are kept in sync by construction; the merged view is a readability aid, not a second execution path.
