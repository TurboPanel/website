...
The TurboPanel API lives in the [turbopanel/turbopanel](https://github.com/turbopanel/turbopanel) repository (`instance/` in local dev). It is a **Hono** application with dual runtimes: **Cloudflare Workers** (`src/workers.ts` + wrangler) and **Deno** (`src/deno.ts` + Unix socket).

## Versioned surfaces

Three audiences each have REST + WebSocket namespaces. Prefixes live in `src/surfaces.ts`. `GET /api/health` is the single unversioned probe.
...
| Admin | `/api/admin/v1/*` | `/ws/admin/v1` | Reserved for future admin UI |
| Daemon | `/api/daemon/v1/*` | `/ws/daemon/v1` | Agent nodes |
...
| Client | `client-routes.ts`, `auth/*` | Sign-in, install wizard, org servers |
| Developer | `developer-routes-core.ts`, `developer-routes.ts` | Fleet, database (Deno-only extras in `developer-routes.ts`) |
| Admin | `admin-routes.ts` | Reserved / legacy admin paths |
| Daemon | `daemon-api-routes.ts`, `daemon-hub.ts` | Agent registration, CA fetch, WS hub |
| Deno-only | `deno.ts`, `system-routes.ts`, `dev-sync.ts`, `tunnel-routes.ts` | Upgrade, reset-dev, dev-sync push |
| Database | `db.ts`, `db/schema.ts` | Drizzle + postgres.js (`prepare: false` for Hyperdrive) |
...
In self-hosted production, Caddy terminates TLS and proxies `/api/*` and `/ws/*` to the Deno Unix socket. In local Tilt dev, Caddy proxies to wrangler (Workers mode) or the dev socket (Deno mode).
...
Client auth routes under `/api/client/v1/auth/*` and `/api/client/v1/install/*`. See the instance repo `AGENTS.md` for the full route table.
...
1. Pick the surface (`client`, `developer`, `admin`, `daemon`).
