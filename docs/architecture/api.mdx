...
Schema changes are versioned in `instance/migrations/` and applied via `pnpm migrate` (Workers deploy and co-located dev converge). Deno dev can still use **drizzle-kit push** (`./sync.sh`) for quick iteration without committing SQL. See `instance/src/lib/db/AGENTS.md`.
...
TurboPanel reserves a top-level `x-turbopanel` key with a `placement.server_id` field. This is a **standard Docker Compose `x-*` vendor extension**, so `docker compose` itself ignores it — the key round-trips untouched through `composeDocumentToRuntimeYaml`.

### Whole-project pinning (today)

`placement.server_id` is read from the **project's base compose** and pins the **entire project — all its environments — to one server**. The value must be a server UUID in the same organization (validated; unknown or stale ids fail cleanly).

### Deploy resolution

`POST /api/client/v1/environments/:id/deploy` accepts an optional request body `serverId`. Resolution order:

| Project pinned? | Body `serverId` | Result |
| --- | --- | --- |
