...
- **App-level `heartbeat`** — sent **only when the agent commit changed** since the last hello/heartbeat, not on every idle tick. On Workers, steady-state heartbeats also perform **no SQLite `cell` write** on the hot path.
...
- Postgres projection (`lastSeenAt`) is throttled to **≤ once/60 s** when agent identity changes; steady-state heartbeats skip DB entirely.
