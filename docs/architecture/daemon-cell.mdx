...
<Steps>
  ...
  <Step>
    **`#initializeFromStorage()`** — calls `#ensureSchema()` (creates SQLite tables), reads
...
Co-located daemons are stored with `remote_address = '__direct__'` so colocated routing and tunnel assignment still work.
