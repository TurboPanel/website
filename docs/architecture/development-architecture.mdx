...
| ------- | ------------ | ----- |
| Caddy HTTPS | 8443 | Browser entrypoint |
...
Browser traffic should use **Caddy on `:8443`**, not wrangler or the raw Expo port.
...
- **Command Pipeline** — Typed command/job records in Postgres (`command` table); Cloudflare Queues (Workers) or RabbitMQ (Deno) for durable async transport; Daemon Cell for live WS delivery and request correlation. Production daemon commands are typed handlers only — never arbitrary shell strings. See `instance/src/lib/commands/` and `daemon/src/instance/commands/`.
