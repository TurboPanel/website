...
This page describes how the TurboPanel **multi-repo** Vagrant environment is orchestrated. Before you start, see [Prerequisites](/docs/development/prerequisites). For day-to-day commands, see [Local development](/docs/getting-started/development) and [Dev console troubleshooting](/docs/getting-started/console-troubleshooting).
...
Paths derive from `TURBOPANEL_DEV_ROOT` (defaults: `$HOME/{turbopaneld,turbopanel,ui,website}`); a per-repo `TURBOPANEL_<DIR>_REPO` variable is an optional override that is only persisted when you set it explicitly (`~/.github` is mounted separately via the `github-repo` Ansible role, not a `TURBOPANEL_*_REPO` variable).
...
Both modes use the `turbopaneld.service` systemd unit. Production code never names the dev checkout root — a CI guard (`deno task check:layout`) and `src/orchestration/paths.test.ts` pin the two trees and the shared `DENO_VERSION` (`2.9.7`).
...
Caddy terminates TLS on **8443** and proxies `/api/*` and `/ws/*` to the instance Unix socket; everything else goes to Expo (dev) or static export (production build mode). Both **`TURBOPANEL_INSTANCE_RUNTIME=deno`** and **`=workers`** sit behind the same Caddy proxy. Every hostname is `https://<host>:8443`. Trust the Platform CA for a name that presents that leaf.
...
1. **Generate + apply migrations:** edit `turbopanel/src/db/schema.ts`, run `pnpm drizzle-kit generate --name <summary>`, review SQL under `migrations/`, then `TURBOPANEL_DATABASE_URL=… pnpm migrate` (or `./scripts/bootstrap-dev-db.sh`).
...
Every table and column, with its type, constraints and a one-sentence description, is in the generated [Database data dictionary](/docs/database) — the same sentences are applied to the live database as `COMMENT ON` metadata.

## Port allocation

| Service | Default port | Host forward | Notes |
| ------- | ------------ | ------------ | ----- |
| Hosting Caddy HTTP | 80 | `0.0.0.0` | Tenant sites, and the control-plane Let's Encrypt window |
| Hosting Caddy HTTPS | 443 | `0.0.0.0` | Hosting only (tenant sites) |
| Caddy HTTPS | 8443 | `0.0.0.0` | Always. Control plane (Platform CA catch-all) |
...
Browser traffic from the host uses **Caddy on `:8443`** (HTTPS). Every hostname is `https://<host>:8443`. Trust the Platform CA. Port **80** answers only while Let's Encrypt is issuing or renewing, and while tenant sites are deployed. Port **443** is hosting. Prefer a LAN hostname when attaching remote test machines to the forwarded `0.0.0.0` ports.
...
- **Drizzle + Postgres** — Schema in `turbopanel/src/db/schema.ts`; Hyperdrive in Workers production.
