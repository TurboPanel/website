...
| `cron[]` | Scheduled jobs, rendered as systemd timers under the service's principal. Tasks created in the console (`POST /tasks`) join these on the same host under one namespace: a task whose name folds to a job name the compose already declares is refused, and compose wins if the two ever collide at deploy |
