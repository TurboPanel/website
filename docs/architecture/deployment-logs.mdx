...
description: The ExecutionLogStore contract, the daemon spool-file durability model, redaction before disk, append/offset read semantics shared by the UI, retention, and the R2 / filesystem / S3 driver matrix
---
...
2. **Retention** — `sweepExpired` rides the **existing** once-a-minute maintenance tick. No new timer, no new connection.
...
| `TURBOPANEL_EXECUTION_LOG_RETENTION_DAYS` | Both runtimes | `30` |
