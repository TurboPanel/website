...
Retransmits mean a packet did not get an ACK in time: congestion, a lossy path, a overloaded receiver, or a middlebox dropping traffic. Brief spikes happen. A rate that stays up is users hitting latency and stalls — pair with [ingress duration](/docs/metrics/ingress#duration) if this host terminates HTTP.
...
- [Ingress](/docs/metrics/ingress) — HTTP-level rates on Caddy or Traefik
