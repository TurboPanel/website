...
- Verify Deno runtime: `/opt/turbopanel/lib/runtime/deno/current/deno --version`
- First run may require **sudo** to create `/opt/turbopanel/lib/runtime` — re-run `./console` after granting privileges.
...
- Confirm the daemon checkout exists: `~/daemon/`.
...
- Workers mode requires `pnpm dev` running manually in `~/instance/`.
...
- Trust `~/instance/certs/ca.crt` (or fetch from `GET /api/daemon/v1/instance/ca`) in your OS or browser.
