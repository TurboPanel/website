...
| **TurboPanel High Availability** | Operators who want TurboPanel to run the panel | Private alpha — [join the waitlist](https://turbopanel.app/sign-up) |
...
<Steps>
  ...
  <Step>
    Confirm the host meets the [Prerequisites](/docs/development/prerequisites) (**Intel Core i7-4790K or equivalent**, **16 GB RAM** minimum / **24 GB+** recommended / **32 GB** ideal), then install [Vagrant](https://developer.hashicorp.com/vagrant) plus a provider (libvirt on Linux, UTM on macOS).
  </Step>
  <Step>
...
When self-hosted is publicly available, you provision a control plane on Debian (the installer refuses other distributions) and enroll servers with the production daemon installer. **Today**, use contributor development only if you are building TurboPanel; otherwise [preview self-hosted docs](/docs/deployment/self-hosted).
...
<Steps>
  ...
  <Step>
    ...
    Run with nothing else, the installer downloads the current release packages (instance, UI and the
    daemon's orchestration tree) from GitHub Releases, verifies every one against its manifest, converges
    the host with the daemon's Ansible roles (Postgres, Redis, RabbitMQ, Docker, certificates, Caddy),
    applies the database migrations and starts the units — see [Control plane](/docs/deployment/control-plane)
    for the flags. It takes no license: the co-located daemon it installs starts without one and
    enrols itself once the wizard has issued the first license, so **this server** goes Online on its own.
  </Step>
  <Step>
...
<Callout type="info" title="Private alpha — join the waitlist">
  TurboPanel High Availability is **not yet publicly available**. [Join the waitlist](https://turbopanel.app/sign-up)
...
Install the daemon on servers that should be managed remotely. The control-plane host already runs its
co-located daemon — never enrol it with this command. Obtain a license from your organization first.
