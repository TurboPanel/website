...
flowchart TD
    ...
    Q -->|Contributing to TurboPanel| Dev[Contributor dev — dev.turbopanel.sh]
...
<Steps>
  <Step>
    ...
    The one-liner clones or updates `~/dev` (or `${TURBOPANEL_DEV_ROOT}/dev`) and launches `./console`.
  </Step>
  <Step>
    `./console` installs the pinned Node runtime (sudo on first run), runs `pnpm install`, and starts the Ink dev console.
  </Step>
  <Step>
    From the console, use **Start dev stack** to install the daemon, bootstrap orchestration, and bring up instance + Caddy + UI via systemd.
  </Step>
  <Step>
...
<Callout type="warn" title="Not dev.turbopanel.sh">
  `curl -fsSL dev.turbopanel.sh | sh` is **contributors only**. It is not the production self-hosted
  install path.
</Callout>
