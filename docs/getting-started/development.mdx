...
| **Software** | [Vagrant](https://developer.hashicorp.com/vagrant) plus a provider (libvirt on Linux, UTM on macOS) |

Install Vagrant and the provider **before** `vagrant up`. Full host tooling, Git/SSH, and provider install notes are on [Prerequisites](/docs/development/prerequisites). Additional Vagrant providers for macOS and Linux are welcome — open a PR against [TurboPanel/dev](https://github.com/TurboPanel/dev).
...
| ---- | ------- | --------- |
| **80** | Hosting HTTP, and Let's Encrypt issuance or renewal | `0.0.0.0` (LAN) |
| **443** | Hosting HTTPS (tenant sites) | `0.0.0.0` (LAN) |
| **8443** | Control plane HTTPS (always) | `0.0.0.0` (LAN) |
...
- **Remote test machines / extra daemons:** give your development host a hostname (for example `https://dev.lan:8443` or your LAN IP) so clients are not stuck on `localhost`. The control plane is always `https://<host>:8443`. Port **80** answers only while Let's Encrypt is issuing or renewing (and while tenant sites are deployed). Port **443** is hosting. Ports `80` / `443` / `8443` / `8081` / `8088` / `19820` listen on all host interfaces. Trust the Platform CA. LAN names can use `curl -k` for a one-off health check.
...
- [Dev console troubleshooting](/docs/getting-started/console-troubleshooting)
