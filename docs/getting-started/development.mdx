...
/opt/turbopanel/
└── vendor/
    ├── deno/2.9.2/        # pinned Deno runtime (orchestration-managed)
...
TurboPanel ships versioned SQL migrations in `instance/migrations/`. Co-located dev converge and Workers deploy apply them via `pnpm migrate`. For quick Deno-only iteration without committing migration files, use drizzle-kit push from `instance/`:
