...
TurboPanel is **not a monorepo**. Clone the five sibling repos into `$HOME` (`~/dev`, `~/daemon`, `~/instance`, `~/ui`, `~/website`). The [turbopanel-dev](https://github.com/turbopanel/turbopanel-dev) repository is the entry point — it installs the dev console and orchestrates the stack via Ansible.
...
```bash
sh scripts/develop.sh   # update turbopanel-dev only
./console
```
...
- Runs `scripts/bootstrap-orchestration.ts` (uv, Python, Ansible under `/opt/turbopanel/lib/runtime/`) and `install-daemon-systemd.sh`
...
/opt/turbopanel/
└── lib/runtime/
    ├── deno/2.9.0/        # pinned Deno runtime (console-managed)
