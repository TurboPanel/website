...
| ---- | ------- | ------- |
| Deno | Console + instance runtime (installed by `./console`) | [deno.com](https://docs.deno.com/runtime/getting_started/installation/) — or let orchestration install the pinned version under `/opt/turbopanel/vendor/deno/<version>/deno` (no `v` prefix) |

Co-located dev on a Debian VM uses the [dev](https://github.com/turbopanel/dev) console (`./console` → **Start dev stack**) with Deno + systemd.
