...
- **Alert webhook** — `PUT /settings/alert-webhook` `{ url }` (or `null` to clear): the instance-wide receiver for `server.offline` and `fleet.mass_disconnect`, kept from before the notifications system existed ([Troubleshooting](/docs/deployment/troubleshooting#getting-paged-when-a-server-goes-dark)). The same events now also reach every administrator's bell and any [notification channel](/docs/using/notifications) a rule routes them to; on self-hosted the webhook may point at a LAN address.
