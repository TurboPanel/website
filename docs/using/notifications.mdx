...
| **Inbox** | Your own rows behind the bell. An organization event lands in the inbox of every member of that organization; an instance event in every instance administrator's. No setup needed. |
| **Channel** | Somewhere outside the console a notification can be delivered. **Yours** (they follow your account across organizations) or the **organization's** (managers edit them). Kinds: `email`, `webhook`, `slack`, `discord`, `telegram`. |
...
| Code | Severity | Scope | When |
| --- | --- | --- | --- |
| `server.offline` | critical | organization | The daemon stopped answering and the server was marked offline. |
| `fleet.mass_disconnect` | critical | instance | A whole sweep lost its fleet at once — usually the control plane's own network or broker. |
| `server.deleted` | info | organization | A server was deleted from the console. |
| `server.daemon_key_revoked` | warning | organization | A server's daemon key was revoked. |
| `access.grant_created` | info | organization | A permission grant was added. |
| `access.grant_revoked` | warning | organization | A permission grant was removed. |
...
<Steps>
  ...
  <Step>
    Enter the **address**: an email address; an `https` URL that accepts a JSON POST; a Slack or Discord incoming-webhook URL; or, for Telegram, your bot token, a slash, then the chat id (`123456:ABC…/987654321`). For a webhook you may add a **signing secret**.
  </Step>
  <Step>
...
One attempt is made at the moment of the event, with a five-second budget per channel. A failure — a non-2xx answer, a timeout, a refused address — is recorded with a short code (`http_503`, `timeout`, `network`, `address_not_public`) and retried by the maintenance tick after **1, 5, 25 and 125 minutes**; after the fifth failure the delivery is **abandoned**. Email deliveries ride the instance's mailer queue (self-hosted: RabbitMQ → the mailer service; High Availability: Mailgun) and are subject to its own retry. The channel row shows the last delivery's status and how many attempts it took.
...
| Inbox fan-out | every organization member (organization event); every instance admin (instance event) |
