...
1. **Changing how you sign in needs a fresh proof.** Enrolling or disabling two-factor, regenerating backup codes, adding or removing a passkey, and unlinking a provider all require **re-authentication**: your current password in the form, or a session younger than **15 minutes**. Otherwise the request is refused (403) and the console asks for the password.
...
| Re-authentication window | 15 minutes, or the current password in the form |
...
| `Reauthentication required` | 403 | The change needs your password or a session under 15 minutes old. |
