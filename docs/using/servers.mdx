...
**Batch update:** on TurboPanel High Availability the fleet rolls out from **Admin → Updates**, and per-server **Update** stays hidden (`updates_managed`). On self-hosted, **Update (n)** is refused with `control_plane_upgrade_required` until **Admin → Updates** has this host's daemon and the control plane on the target build. After that gate opens, selected servers with an update available can move together; the co-located server is not in that list.
...
- **Daemon update** — when a newer build is published and the platform gate is open, **Update** runs the daemon's own updater over the connection; **Clear stuck update** resets an update the daemon never reported back. While the gate is shut the badge reads **Waiting for the control plane upgrade**. On TurboPanel High Availability the button is hidden. Co-located hosts are updated from **Admin → Updates**.
...
| `control_plane_upgrade_required` | 409 | Self-hosted: the fleet gate is shut. The badge reads **Waiting for the control plane upgrade**. |
| `updates_managed` | 409 | TurboPanel High Availability: per-server **Update** is hidden. Use **Admin → Updates**. |
