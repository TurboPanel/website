...
| `compose_field_requires_org_opt_in` | 403 | A gated key (`privileged`, `cap_add`, …) and the organization has not opted in. An owner enables it with `PUT /organizations/:id/compose-privileged-fields`. |
