...
The daemon trust model is **identical** — only the control plane URL and TLS trust root differ: hosted edge TLS, or self-hosted Platform CA **by default** with an uploaded or Let's Encrypt certificate using the system trust store instead. See [Control plane TLS modes](/docs/deployment/control-plane#control-plane-tls-modes).
...
**Hosting is separate:** tenant **hosting** leaves (Caddy-fronted web services) are operator-pinned library certificates, Caddy `tls internal`, or a Let's Encrypt row Caddy issues on the serving host (`tlsMode: acme`). They are never issued by the Organization CA. See [Hostnames and TLS](/docs/deployment/hostnames-and-tls).

## Related

- [Trust boundaries](/docs/security/trust-boundaries) — what each part reaches if it is compromised
