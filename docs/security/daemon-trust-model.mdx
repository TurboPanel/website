...
**Future:** tenant **hosting** leaves (Caddy-fronted web services) are operator-pinned library certificates, Caddy `tls internal`, or a Let's Encrypt row Caddy issues on the serving host (`tlsMode: acme`). They are never issued by the Organization CA. See [Hostnames and TLS](/docs/deployment/hostnames-and-tls).
