...
The daemon trust model is **identical** — only the control plane URL and TLS trust root differ: hosted edge TLS, or self-hosted Platform CA on `:8443` with an uploaded or Let's Encrypt hostname using the system trust store instead. See [Control plane listeners](/docs/deployment/control-plane#control-plane-listeners).
